...
## Word dropout
losses_file_name = "3word_dropout"
save_losses_path = Path(losses_save_path) / losses_file_name
with open(save_losses_path, 'rb') as file:
...
## MDR
losses_file_name = "6freebits_worddropout_mdr"
save_losses_path = Path(losses_save_path) / losses_file_name
with open(save_losses_path, 'rb') as file:
...
## Freebits
losses_file_name = "5freebits_dropout"
save_losses_path = Path(losses_save_path) / losses_file_name
with open(save_losses_path, 'rb') as file:
    (lists_fb, print_every, args) = pickle.load(file)
print(print_every)
...
nll_fb_smooth = interpolate.make_interp_spline(x, lists_fb[0])(x_smooth)
...
ax1.plot(x_smooth, nll_fb_smooth, '--', label = 'Freebits - NLL', color='green')
...
kl_fb_smooth = interpolate.make_interp_spline(x, lists_fb[1])(x_smooth)
...
ax2.plot(x_smooth, kl_fb_smooth, label = 'Freebits - KL', color='green')
